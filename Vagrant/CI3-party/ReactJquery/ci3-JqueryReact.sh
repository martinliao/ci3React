...
sed -i "s/\(.*config\['unauthorized_route'\][ ]\).*/\1= 'page';/g" ${DOCROOT}/application/third_party/SmartyAcl/config/smarty_acl.ph
###     12. 改 cache_settings - driver 為 redis
echo "\$config['cache_settings']['driver'] = 'redis';" | tee -a ${DOCROOT}/application/third_party/SmartyAcl/config/smarty_acl.ph
