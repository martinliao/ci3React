...
cp -r /CI3Party/SmartyACL/modules/Admin ${DOCROOT}/application/modules/
cp -r /CI3Party/SmartyACL/modules/AuthAdmin ${DOCROOT}/application/modules/
cp -r /CI3Party/SmartyACL/views/* ${DOCROOT}/application/views/
...
## 0. 改預設 base_url. (在 config['base_url'] 後加入 )
sed -i "/.*config\['base_url'\] = '';/a\$config['base_url'] = rtrim(\$baseurl, '/');" config.php
sed -i "/.*config\['base_url'\] = '';/a\$baseurl .= str_replace(basename(\$_SERVER['SCRIPT_NAME']),\"\",\$_SERVER['SCRIPT_NAME']);" config.php
sed -i "/.*config\['base_url'\] = '';/a\$baseurl  = \"http://\".\$_SERVER['HTTP_HOST'];" config.php
## 0. 把空的 base_url 刪除.
sed -i "/.*config\['base_url'\][ ]= '';.*/d" ${DOCROOT}/application/config/config.php
echo "*** Login/Logout, done"

### 3. 加 Auth
cp -r /CI3Party/SmartyACL/modules/Auth ${DOCROOT}/application/modules/
# 改 welcome
...
cp -r /CI3Party/SmartyACL/views/account.php ${DOCROOT}/application/views/
echo "*** Auth模組, done"

### 4. Modules模組
cp -r /CI3Party/SmartyACL/modules/Admin/views/modules.php ${DOCROOT}/application/modules/Admin/views/
cp -r /CI3Party/SmartyACL/modules/Admin/views/modules_form.php ${DOCROOT}/application/modules/Admin/views/
#     4-b. 加路由; Modules
cat <<EOF | tee -a ${DOCROOT}/application/config/routes.php
//Modules
...
### 5. Roles角色
cp -r /CI3Party/SmartyACL/modules/Admin/views/roles.php ${DOCROOT}/application/modules/Admin/views/
cp -r /CI3Party/SmartyACL/modules/Admin/views/roles_form.php ${DOCROOT}/application/modules/Admin/views/
#     5-b. 加路由; Roles
cat <<EOF | tee -a ${DOCROOT}/application/config/routes.php
//Roles
...
### 6. Admins
cp -r /CI3Party/SmartyACL/modules/Admin/views/admins.php ${DOCROOT}/application/modules/Admin/views/
cp -r /CI3Party/SmartyACL/modules/Admin/views/admins_form.php ${DOCROOT}/application/modules/Admin/views/
#     6-b. 加路由; Admins
cat <<EOF | tee -a ${DOCROOT}/application/config/routes.php
//Admins
...
### 7. Users一般使用者
cp -r /CI3Party/SmartyACL/modules/Admin/views/users.php ${DOCROOT}/application/modules/Admin/views/
cp -r /CI3Party/SmartyACL/modules/Admin/views/users_form.php ${DOCROOT}/application/modules/Admin/views/
#     7-b. 加路由; Users
cat <<EOF | tee -a ${DOCROOT}/application/config/routes.php
//Users
